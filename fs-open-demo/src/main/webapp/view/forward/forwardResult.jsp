...
<body>
APP 跳转到第三方 : ${message} <br/>
获取的用户的openUserId为: ${findstatus == '0' ? openUser.openUserId:'未获取到用户'}
<br />
<input type="button" id="http_test" class="form_button" style="padding:8px;font: 13.3333px Arial;margin-left: 15px;border:1px solid;height:33px;cursor: pointer; width:150px;background:#269CE9;color:#FFF" value="获取用户信息" />
		...
		if(true || '${findstatus}' == '0') {
			...
			$("#http_test").click(function() {
				...
				$.ajax({
			        type: "POST",
			        url: url_send,
			        data:{'openUserId':'${openUser.openUserId}'}, 
			...
			$("#send_test").click(function() {
				...
				$.ajax({
			        type: "POST",
			        url: url_send,
			        data:{msgContent:'Test 中文',openUserIds:'${openUser.openUserId}'},
			        success: function(data) {
							var content = data.rs;
							if (typeof(content) == 'object') {
								content = JSON.stringify(content);
			...
			$("#send_template_test").click(function() {
				...
				$.ajax({
			        type: "POST",
			        url: url_send,
			        data:{openUserIds:'${openUser.openUserId}'},
			        success: function(data) {
							var content = data.rs;
							if (typeof(content) == 'object') {
								content = JSON.stringify(content);
