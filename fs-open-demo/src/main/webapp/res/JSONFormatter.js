  ...
  var init = function( json, options ) {
    ...
    loopObjectOfObjects = function(json2, ulId) {
      $.each(json2, function(k3, v3) {
        // object of objects
        if(typeof v3 == 'object') {
          $('#' + settings.list_id + settings.num + ' #' + ulId).append('<li><span>{</span> <ul id="' + ulId + '-' + k3 + '"></ul></li>');
          $.each(v3, function(k4, v4) {
            if(typeof v4 == 'object' && v4 != null) {
              $('#' + settings.list_id + settings.num + ' #' + ulId + '-' + k3).append('<li>' + k4 + ' <span>{</span> <ul id="'+k4+'-'+loopCount+'"></ul></li>');
              loopAgain(v4, k4, k4 + '-' + loopCount);
            }
            else {
              $('#' + settings.list_id + settings.num + ' #' + ulId + '-' + k3).append('<li>' + k4 + ': ' + v4 + '</li>');
            }
          ...
          // normal array
          $('#' + settings.list_id + settings.num +' #' + ulId).append('<li>' + v3 + '</li>')
        }
      });
    },
      ...
      $.each(v, function(nextKey, nextVal) {
        ...
        if(nextVal != null && typeof nextVal == 'object') {
          if(nextVal.length == 0) {
            ...
            $('#' + settings.list_id + settings.num + ' #' + ulId).append('<li><i>' + nextKey + ':</i> []</li>');
          }
          else if(nextVal.length >= 1) {
            // an object of objects
            $('#' + settings.list_id + settings.num +' #' + ulId).append('<li><b>' + nextKey + ':</b> <span>[</span> ' + newList + '</li>');
          ...
          // }
          // else {
            $('#' + settings.list_id + settings.num + ' #' + ulId).append('<li><i>'+ nextKey + ':</i> ' + nextVal + '</li>');
    ...
    addClosingBraces = function() {
      $('#' + settings.list_id + settings.num + ' span').each(function() {
    ...
    var jsonList = $('<ul id="' + settings.list_id + settings.num + '" />');
    ...
    $.each(json, function(key, val) {
      ...
      if(val != null && typeof val == 'object') {
        ...
        if(goObj) {
          $('#' + settings.list_id+settings.num).append('<li><b>' + key + ':</b> <span>[</span><ul id="' + nk + '-' + loopCount + '"></ul></li>');
          ...
          $('#' + settings.list_id+settings.num).append('<li><b>' + key + ':</b> <span>{</span><ul id="' + key + '-' + loopCount + '"></ul></li>');
          loopAgain(val, key, key + '-' + loopCount);
        }

      }
      else {
        $('#' + settings.list_id + settings.num).append('<li><i>' + key + ':</i> ' + val + '</li>');
      }
    });
    ...
    if(settings.collapse) {
      addToggles(settings.list_id + settings.num);
    }
